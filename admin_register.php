...
if(isset($_POST['submit'])){
   ...
   if($select->rowCount() > 0){
      ...
      if($pass != $cpass){
         ...
         $insert = $conn->prepare("INSERT INTO `users`(name, email, number, state, district, sector, password, user_type, image) VALUES(?,?,?,?,?,?,?,?,?)");
         $insert->execute([$name, $email, $number, $state, $district, $sector, $pass, $user_type, $image]);
...
<section class="form-container">
   ...
   <form action="" enctype="multipart/form-data" method="POST">
      ...
      <select name="user_type" class="box" required>
            <option value="" selected disabled>select user_type</option>
               <option value="admin">admin</option>
               <option value="farmer">farmer</option>
         </select>
      <input type="file" name="image" class="box" required accept="image/jpg, image/jpeg, image/png">
      <input type="submit" value="register now" class="btn" name="submit">
      <p><a href="admin_page.php">Back to Admin Panel</a></p>
   </form>
